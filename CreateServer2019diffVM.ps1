...
$ParentVMVHDPath = "S:\Parent VHD\Server 2019\Server 2019 parent Disk.vhdx" #Base for Differencing Image

New-VHD -ParentPath $ParentVMVHDPath -Path $VMVHDPath -Differencing
Write-Output("Created Differencing Disk with name " + $VMVHDPath)

New-VM -Name $VMName -MemoryStartupBytes 4GB -Generation 2 -BootDevice VHD -VHDPath $VMVHDPath -Switch "External Virtual Switch" 
